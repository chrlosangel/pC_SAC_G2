...
//  void SetSegLengths(char* cStartEndFile);
  ...
  unordered_map<int,CXYVector<float> > GrowChainSegment_GeneratePotentialPoints(void);
...
//  void UpdateMap_TreeIt_MultiArray_Clean();
  ...
  void GrowChainSegment_WritePtsArr(tree< CXYVector<float> >::iterator lastNode, int chainInd);
  ...
  void FindContactStartEndNodeIndFromNodeInd_2(int iNodeInd, vector<vector<int> > & vStartEndInd, int iChain);
  float GrowChainSegment_GetTwoNodesDistFromProfile_2 ( int iLeftNodeInd, int iRightNodeInd, int iChain);
  ...
  void GetNodeIndAndDistFromNodeInd_2 (int iNodeInd, vector<int> & rvConInd, vector<float> & rvConDist, int iChain );
...
private:
  ...
  deque<tree<CXYVector<float> >::iterator> m_qPrvNodes; //stores the last node iterator of each mMax chain
...
//  int m_iNumMiddleEndPoints;
  ...
  map<tree<CXYVector<float>>::iterator, float, tree<CXYVector<float> >::iterator_base_less> m_Weight_map;
  ...
  //--------------------------------------------------------
  deque<tree<CXYVector<float> >::iterator> m_qPrvNodes_2; //stores the last node iterator of each mMax chain
  tree< CXYVector<float> >* m_pTChain_2; // tree stored coordinates
  map<tree<CXYVector<float> >::iterator, int, tree<CXYVector<float> >::iterator_base_less> m_repeatNode; //stores how many times the current chain blocked on certain nodes before having to go back
  map<tree<CXYVector<float> >::iterator, int, tree<CXYVector<float> >::iterator_base_less> m_reminiscentRepeatNode; //stores how many times the chain had to go back because of one node
  int m_numInteraction = 0; //number of interactions in chain
  CXYMatrix<float> GetAbsoluteNodeSamples(
    tree<CXYVector<float> >::iterator  &ritNode,
    CXYMatrix<float> kMSamplesOrg);
  void GrowChainSegment_RandSelect_2(
    ...
    float& fSumBeta_t,
    float& fSumGamma_t);
  void GrowChainSegment_Single_RandSelect_2(
    ...
    float& fSumBeta_t,
    float& fSumGamma_t);
  void GrowChainSegment_CalBeta_t_2_Single(
    CXYMatrix<float>& kMSamples,
    tree<CXYVector<float> >::iterator& itPrvNode,
    int iNodeInd,
    vector<FloatIntPair>& vBeta_t,
    ...
    int iChain);
  void GrowChainSegment_h1Function_2_mod(
    CXYVector<float>& kV_Vector_cur,
    tree< CXYVector<float> >::iterator itPrvNode,
    int iSegInd);
  void GrowChainSegment_h2Function_2_mod(
    CXYVector<float>& kV_Vector_cur,
    vector<int>& rvConInd,
    vector<float>& rvConDist,
    CXYMatrix<float>& rKM_PrvNodePos);
  void GrowChainSegment_h3Function_2_mod(
    CXYVector<float>& kV_Vector_cur,
    int iSegInd,
    vector<vector<int > >& vStartEndInd,
    CXYMatrix<float>& rKM_PrvNodePos,
  ...
  void AddNewNodes(
    unordered_map<int,
    CXYVector<float> > &potNodes);
  void DistributeInteractions(int iChainInd);
  void ClearDistribution();
  void ConfirmDistribution();
  void StartNewChain(int iChain);
  void GrowChainSegment_ConfirmDistribution_2(tree<CXYVector<float>>::iterator lastNode);
  float m_fthreshold_A, m_fthreshold_B; //respectively thresholds for colliding constraints and interaction constraints
  int m_writtenFiles=0; //how many chains are already done and saved
  vector<int> m_MSegSegPval_2_distributed; //stores the target distribution
  vector<int> necessary_distribution; //indicates for each interactions in the distribution if it was forced or random
  const int m_nb_features = 9; //number of columns in the vectors contained in the tree
  int m_nb_full_reset=0; //how many times the chain has restarted from the beginning
  int m_iteration = 0;
  int m_nb_nodes = 0; //number of nodes a chain is supposed to have
  int m_interaction_distance = 800;
  map<tree<CXYVector<float>>::iterator, float, tree<CXYVector<float> >::iterator_base_less> m_Iteration_map; //stores how many interactions were needed for each chain
  float mfmc_coeff = 0.2; //monte carlo probability 
